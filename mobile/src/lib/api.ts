...
export type RoomCard = { room_id: string; sport: string; title: string | null; venue: { name: string; address: string | null }; schedule: { start_at: string; end_at: string }; capacity: { available_public_slots: number; required_slots: number }; host: { user_id: string; display_name: string }; badges: string[] };
export type RoomDetail = { id: string; sport: string; status: string; venue: { name: string; address: string | null; latitude?: number | null; longitude?: number | null }; schedule: { start_at: string; end_at: string }; capacity: { total: number; available_public_slots: number }; host: { id: string; display_name: string; avatar_url: string | null; sport_profile: unknown | null }; viewer: null | { is_host: boolean; schedule_conflict: boolean; can_request_join: boolean; application: null | { id: string; status: string }; participant: null | { id: string; status: string }; available_actions: string[] } };
...
export type CreateRoomInput = { sport_code: string; title: string | null; venue: { name: string; address: string | null; latitude?: number | null; longitude?: number | null }; scheduled_start_at: string; scheduled_end_at: string; capacity: number; host_participates: boolean; reserved_external_count: number; price_amount: number | null; currency: 'VND'; preferred_skill: { min_score?: number; max_score?: number } | null; equipment: { supply_mode: 'HOST_PROVIDES' | 'PLAYER_BRINGS' | 'MIXED' | 'NOT_APPLICABLE'; quantity_per_participant?: number | null; notes?: string | null; allowed_options: { display_name: string }[] }; allow_emergency_replacement: boolean };
...
class ApiClient {
  ...
  private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    try {
      response = await fetch(`${API_BASE_URL}${path}`, { ...init, headers: { Accept: 'application/json', ...(init.body ? { 'Content-Type': 'application/json' } : {}), ...(mutation && path !== '/api/v1/auth/register' && path !== '/api/v1/auth/login' && path !== '/api/v1/auth/logout' ? { 'Idempotency-Key': `${Date.now()}-${Math.random().toString(36).slice(2)}` } : {}), ...(this.token ? { Authorization: `Bearer ${this.token}` } : {}), ...init.headers } });
    } catch {
