...
export type RoomCard = { room_id: string; sport: string; title: string | null; venue: { name: string; address: string | null }; schedule: { start_at: string; end_at: string }; capacity: { available_public_slots: number; required_slots: number }; cover_image_url: string | null; host: { user_id: string; display_name: string }; badges: string[] };
export type RoomDetail = { id: string; sport: string; status: string; venue: { name: string; address: string | null; latitude?: number | null; longitude?: number | null }; images: RoomImage[]; schedule: { start_at: string; end_at: string }; capacity: { total: number; available_public_slots: number }; host: { id: string; display_name: string; avatar_url: string | null; sport_profile: unknown | null }; viewer: null | { is_host: boolean; schedule_conflict: boolean; can_request_join: boolean; application: null | { id: string; status: string }; participant: null | { id: string; status: string }; available_actions: string[] } };
