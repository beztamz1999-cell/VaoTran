...
import { Alert, Linking, Platform, Pressable, SafeAreaView, ScrollView, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
import { Image } from 'expo-image';
import * as ImagePicker from 'expo-image-picker';
import * as ImageManipulator from 'expo-image-manipulator';
import { API_BASE_URL, api, friendlyError, type HostManager as HostManagerData, type Matches, type Me, type RoomCard, type RoomDetail } from '@/lib/api';
...
import { SkillRangeSelector } from '@/components/skill-range-selector';
import { skillRangeLabel, skillRangesOverlap, type SkillRange } from '@/lib/skill-range';
import { formatParticipationFee, parseVndInput } from '@/lib/fees';
...
const imageUrl = (url: string) => url.startsWith('http') ? url : `${API_BASE_URL}${url}`;
const fromApiSkillRange = (range: { min_score: number | null; max_score: number | null } | null | undefined): SkillRange => range?.min_score !== null && range?.min_score !== undefined && range.max_score !== null && range.max_score !== undefined ? { minScore: range.min_score, maxScore: range.max_score } : null;
...
function Discovery({ me }: { me: Me }) {
  const [sport, setSport] = useState('BADMINTON'); const [rooms, setRooms] = useState<RoomCard[]>([]); const [detail, setDetail] = useState<RoomDetail | null>(null); const [busy, setBusy] = useState(true); const [error, setError] = useState(''); const [filter, setFilter] = useState(false); const [skillFilter, setSkillFilter] = useState<SkillRange>(null);
  useEffect(() => { void (async () => { setBusy(true); try { const results = await api.search(sport); setRooms(results.filter((room) => skillRangesOverlap(fromApiSkillRange(room.preferred_skill), skillFilter))); setError(''); } catch (e) { setError(friendlyError(e)); } finally { setBusy(false); } })(); }, [sport, skillFilter]);
  if (detail) return <RoomDetailScreen room={detail} back={() => setDetail(null)} refresh={async () => setDetail(await api.room(detail.id))} />;
  if (filter) return <Filters value={skillFilter} onApply={setSkillFilter} back={() => setFilter(false)} />;
...
const venueImageS = StyleSheet.create({ thumbImage: { width: 70, minHeight: 76, borderRadius: 12, backgroundColor: colors.brandSoft }, gallery: { marginHorizontal: -space.lg }, heroImage: { width: 428, height: 220, borderRadius: radius.hero, marginHorizontal: space.lg, backgroundColor: colors.brandSoft } });
function RoomCardView({ room, onPress }: { room: RoomCard; onPress: () => void }) { return <Pressable onPress={onPress} style={s.roomCard}>{room.cover_image_url ? <Image source={{ uri: imageUrl(room.cover_image_url) }} style={venueImageS.thumbImage} contentFit="cover" /> : <View style={s.thumb}><Text style={s.thumbText}>⌁</Text></View>}<View style={{ flex: 1, gap: 4 }}><View style={s.cardTop}><Text style={s.meta}>{date(room.schedule.start_at)}</Text><StatusBadge label={`Còn ${room.capacity.available_public_slots}/${room.capacity.required_slots}`} tone="orange" /></View><Text style={s.roomTitle}>{room.title ?? room.venue.name}</Text><Text style={s.meta}>{room.venue.address ?? room.venue.name}</Text><Text style={s.small}>Trình độ · {skillRangeLabel(fromApiSkillRange(room.preferred_skill))}</Text><Text style={s.feeCard}>{formatParticipationFee(room.participation_fee_per_person)}</Text></View></Pressable>; }

function RoomDetailScreen({ room, back, refresh }: { room: RoomDetail; back: () => void; refresh: () => Promise<void> }) { const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const actions = room.viewer?.available_actions ?? []; const join = async () => { setBusy(true); try { if (actions.includes('WITHDRAW_APPLICATION')) await api.withdraw(room.viewer!.application!.id); else await api.requestJoin(room.id); await refresh(); } catch (e) { setError(friendlyError(e)); } finally { setBusy(false); } }; const label = actions.includes('WITHDRAW_APPLICATION') ? 'RÚT YÊU CẦU' : actions.includes('REQUEST_JOIN') ? 'YÊU CẦU THAM GIA' : null; return <ScrollView contentContainerStyle={s.content}><Pressable onPress={back}><Text style={s.back}>‹</Text></Pressable>{room.images.length ? <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={venueImageS.gallery}>{room.images.map(image => <Image key={image.id} source={{ uri: imageUrl(image.url) }} style={venueImageS.heroImage} contentFit="cover" />)}</ScrollView> : <View style={s.hero}><Text style={s.heroIcon}>⌁</Text><StatusBadge label={`Còn ${room.capacity.available_public_slots}/${room.capacity.total} chỗ`} /></View>}<Text style={s.meta}>{date(room.schedule.start_at)} - {new Date(room.schedule.end_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</Text><Text style={s.detailTitle}>{room.venue.name}</Text><Text style={s.meta}>⌖ {room.venue.address ?? 'Chưa cập nhật địa chỉ'}</Text><SecondaryButton label="MỞ VỊ TRÍ TRÊN GOOGLE MAPS" onPress={() => openGoogleMaps(room.venue.name, room.venue.address, room.venue.latitude, room.venue.longitude)} />{error && <ErrorBanner message={error} />}<Card><SectionHeader title="Thông tin trận" /><Info label="Trình độ" value={skillRangeLabel(fromApiSkillRange(room.preferred_skill))} /><Info label="Phí tham gia" value={formatParticipationFee(room.participation_fee_per_person)} /><Info label="Sức chứa" value={`${room.capacity.total} người`} /><Info label="Trạng thái" value={status(room.status)} /></Card><Card><Text style={s.section}>Chủ sân</Text><View style={s.host}><Avatar name={room.host.display_name} /><View><Text style={s.roomTitle}>{room.host.display_name}</Text><Text style={s.meta}>Host của trận này</Text></View></View></Card>{room.viewer?.application && <StatusBadge label={status(room.viewer.application.status)} tone="orange" />}{label && <View style={s.joinBlock}><Text style={s.feeJoin}>Phí tham gia · {formatParticipationFee(room.participation_fee_per_person)}</Text><PrimaryButton label={label} disabled={busy} onPress={() => void join()} /></View>}{actions.includes('OPEN_HOST_MANAGER') && <HostManager roomId={room.id} />}</ScrollView>; }
...
function Filters({ back, value, onApply }: { back: () => void; value: SkillRange; onApply: (range: SkillRange) => void }) { const [open, setOpen] = useState(true); const [skillRange, setSkillRange] = useState<SkillRange>(value); return <ScrollView contentContainerStyle={s.content}><View style={s.top}><Pressable onPress={back}><Text style={s.back}>‹</Text></Pressable><Text style={s.screen}>Bộ lọc tìm trận</Text><Pressable onPress={() => setSkillRange(null)}><Text style={s.link}>Xóa bộ lọc</Text></Pressable></View><Filter title="Thời gian" labels={['Hôm nay', 'Ngày mai', '7 ngày tới']} /><SkillRangeSelector value={skillRange} onChange={setSkillRange} label="Trình độ" helperText="Lọc theo khoảng trình độ phù hợp của trận." /><Filter title="Khu vực" labels={['Việt Yên', 'Bắc Ninh', 'Bắc Giang']} /><View style={s.switchRow}><Text style={s.small}>Chỉ hiển thị trận còn chỗ</Text><Switch value={open} onValueChange={setOpen} trackColor={{ true: colors.brand }} /></View><PrimaryButton label="ÁP DỤNG BỘ LỌC" onPress={() => { onApply(skillRange); back(); }} /></ScrollView>; }
...
function Create() {
  ...
  const [selectedImages, setSelectedImages] = useState<{ uri: string; mimeType?: string | null }[]>([]);
  ...
  const [skillRange, setSkillRange] = useState<SkillRange>({ minScore: 4, maxScore: 6 });
  ...
  const pickVenueImage = async () => {
    ...
    const resized = await ImageManipulator.manipulateAsync(asset.uri, [{ resize: { width: 1600 } }], { compress: 0.82, format: ImageManipulator.SaveFormat.JPEG });
    setSelectedImages(current => [...current, { uri: resized.uri, mimeType: 'image/jpeg' }]);
  };
  const create = async () => {
    ...
    try {
      setCreated(await api.createRoom({
        ...
        reserved_external_count: reserved, price_amount: null, participation_fee_per_person: participationFee, currency: 'VND', preferred_skill: skillRange ? { min_score: skillRange.minScore, max_score: skillRange.maxScore } : null,
        equipment: {
  ...
  return <ScrollView contentContainerStyle={s.content}>
    ...
    {step === 1 ? <View style={s.form}>
      ...
      <VenueImagePicker images={selectedImages} onAdd={() => void pickVenueImage()} onRemove={(uri) => setSelectedImages(current => current.filter(image => image.uri !== uri))} />
      <Stepper label="Sức chứa" value={capacity} set={setCapacity} min={1} />
      <ParticipationFeeField value={participationFee} onChange={setParticipationFee} />
      ...
      <SkillRangeSelector value={skillRange} onChange={setSkillRange} helperText="Đây là gợi ý để người chơi tham khảo; HOST vẫn quyết định yêu cầu tham gia." />
      ...
      {created ? <View style={s.form}><StatusBadge label="Đã tạo bản nháp" /><PrimaryButton label="ĐĂNG TRẬN" disabled={busy} onPress={() => void (async () => { try { setBusy(true); for (const image of selectedImages) await api.uploadRoomImage(created.room_id, image.uri, image.mimeType ?? 'image/jpeg'); await api.publishRoom(created.room_id, created.version); Alert.alert('Đã đăng trận', 'Trận đã xuất hiện trong danh sách của bạn.'); } catch (e) { setError(friendlyError(e)); } finally { setBusy(false); } })()} /><HostManager roomId={created.room_id} /></View> : <PrimaryButton label="TẠO TRẬN" disabled={busy} onPress={() => void create()} />}
...
const pickerS = StyleSheet.create({ imageSection: { gap: 8 }, imageRow: { flexDirection: 'row', gap: 9 }, imageAdd: { width: 88, height: 72, borderRadius: radius.input, borderWidth: 1, borderStyle: 'dashed', borderColor: colors.brand, justifyContent: 'center', alignItems: 'center', backgroundColor: colors.brandSoft }, imageAddText: { color: colors.brandStrong, fontSize: 12, fontWeight: '900', textAlign: 'center' }, imageThumb: { width: 88, height: 72, borderRadius: radius.input }, removeImage: { position: 'absolute', right: 4, top: 4, width: 22, height: 22, borderRadius: 11, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' }, removeText: { color: colors.error, fontWeight: '900' } });
function VenueImagePicker({ images, onAdd, onRemove }: { images: { uri: string }[]; onAdd: () => void; onRemove: (uri: string) => void }) { return <View style={pickerS.imageSection}><Text style={s.section}>Hình ảnh sân</Text><Text style={s.meta}>Giúp người chơi dễ nhận biết địa điểm · tối đa 3 ảnh</Text><View style={pickerS.imageRow}>{images.map((image, index) => <View key={image.uri}><Image source={{ uri: image.uri }} style={pickerS.imageThumb} contentFit="cover" /><Pressable style={pickerS.removeImage} onPress={() => onRemove(image.uri)}><Text style={pickerS.removeText}>×</Text></Pressable>{index === 0 && <StatusBadge label="Ảnh bìa" />}</View>)}{images.length < 3 && <Pressable style={pickerS.imageAdd} onPress={onAdd}><Text style={pickerS.imageAddText}>＋{`\n`}Thêm ảnh</Text></Pressable>}</View></View>; }
...
function ParticipationFeeField({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return <View style={feeS.wrap}>
    <View style={feeS.head}><View><Text style={s.section}>Phí tham gia</Text><Text style={s.meta}>Chi phí dự kiến cho mỗi người tham gia</Text></View><Pressable onPress={() => onChange(0)} style={[feeS.free, value === 0 && feeS.freeActive]}><Text style={[feeS.freeText, value === 0 && feeS.freeTextActive]}>Miễn phí</Text></Pressable></View>
    <View style={feeS.inputRow}><TextInput value={value === 0 ? '' : new Intl.NumberFormat('vi-VN').format(value)} onChangeText={(text) => onChange(parseVndInput(text))} keyboardType="numeric" placeholder="50.000" placeholderTextColor="#9AA39F" style={feeS.input} /><Text style={feeS.unit}>đ / người</Text></View>
    <Text style={feeS.preview}>{formatParticipationFee(value)}</Text>
  </View>;
}

const feeS = StyleSheet.create({
  wrap: { gap: 8, padding: 14, borderRadius: radius.card, borderWidth: 1, borderColor: colors.border, backgroundColor: '#fff' }, head: { flexDirection: 'row', gap: 12, alignItems: 'flex-start', justifyContent: 'space-between' }, free: { borderRadius: radius.pill, borderWidth: 1, borderColor: colors.border, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: '#fff' }, freeActive: { borderColor: colors.brand, backgroundColor: colors.brandSoft }, freeText: { color: colors.muted, fontSize: 12, fontWeight: '800' }, freeTextActive: { color: colors.brandStrong }, inputRow: { minHeight: 50, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: colors.border, borderRadius: radius.input, paddingLeft: 13, backgroundColor: colors.section }, input: { flex: 1, color: colors.text, fontSize: 16, fontWeight: '800', minHeight: 48 }, unit: { color: colors.muted, fontSize: 13, fontWeight: '700', paddingHorizontal: 13 }, preview: { color: colors.brandStrong, fontSize: 13, fontWeight: '800' },
});
...
function HostManagerView({ roomId, label = 'Mở quản lý trận' }: { roomId: string; label?: string }) {
  ...
  return <View style={s.manager}>
    ...
    <View style={s.feeManager}><View><Text style={s.meta}>Phí tham gia</Text><Text style={s.feeCard}>{formatParticipationFee(data.participation_fee_per_person)}</Text></View><Pressable onPress={() => { setFeeDraft(data.participation_fee_per_person); setEditingFee(!editingFee); }}><Text style={s.link}>{editingFee ? 'Hủy' : 'Sửa'}</Text></Pressable></View>
    {editingFee && <View style={feeS.wrap}><Text style={s.meta}>Mức phí chỉ để mọi người chủ động chuẩn bị; VàoTrận không thu tiền.</Text><View style={feeS.inputRow}><TextInput value={feeDraft === 0 ? '' : new Intl.NumberFormat('vi-VN').format(feeDraft)} onChangeText={(text) => setFeeDraft(parseVndInput(text))} keyboardType="numeric" placeholder="50.000" placeholderTextColor="#9AA39F" style={feeS.input} /><Text style={feeS.unit}>đ / người</Text></View><View style={s.actions}><SecondaryButton label="MIỄN PHÍ" disabled={busy} onPress={() => setFeeDraft(0)} /><PrimaryButton label="LƯU PHÍ" disabled={busy} style={{ flex: 1 }} onPress={() => void act(async () => { await api.updateRoom(roomId, { participation_fee_per_person: feeDraft }); setEditingFee(false); })} /></View></View>}
    ...
    <HostImageManager roomId={roomId} images={data.images} reload={load} />
...
function HostImageManager({ roomId, images, reload }: { roomId: string; images: RoomDetail['images']; reload: () => Promise<void> }) { const [busy, setBusy] = useState(false); const add = async () => { const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], allowsMultipleSelection: false, quality: 0.85 }); if (result.canceled || !result.assets[0]) return; setBusy(true); try { const resized = await ImageManipulator.manipulateAsync(result.assets[0].uri, [{ resize: { width: 1600 } }], { compress: 0.82, format: ImageManipulator.SaveFormat.JPEG }); await api.uploadRoomImage(roomId, resized.uri); await reload(); } catch (error) { Alert.alert('Không thể tải ảnh', friendlyError(error)); } finally { setBusy(false); } }; return <View style={pickerS.imageSection}><SectionHeader title="Hình ảnh sân" />{images.length ? <ScrollView horizontal contentContainerStyle={pickerS.imageRow}>{images.map(image => <View key={image.id}><Image source={{ uri: imageUrl(image.url) }} style={pickerS.imageThumb} contentFit="cover" /><Pressable style={pickerS.removeImage} disabled={busy} onPress={() => void (async () => { setBusy(true); try { await api.deleteRoomImage(roomId, image.id); await reload(); } finally { setBusy(false); } })()}><Text style={pickerS.removeText}>×</Text></Pressable><Pressable onPress={() => void (async () => { setBusy(true); try { await api.setRoomImageCover(roomId, image.id); await reload(); } finally { setBusy(false); } })()}><Text style={s.link}>{image.isCover ? 'Ảnh bìa' : 'Đặt bìa'}</Text></Pressable></View>)}{images.length < 3 && <Pressable style={pickerS.imageAdd} disabled={busy} onPress={() => void add()}><Text style={pickerS.imageAddText}>＋{`\n`}Thêm ảnh</Text></Pressable>}</ScrollView> : <Pressable style={pickerS.imageAdd} disabled={busy} onPress={() => void add()}><Text style={pickerS.imageAddText}>＋{`\n`}Thêm ảnh sân</Text></Pressable>}</View>; }
