...
const createRoomSchema = z.object({
  ...
  participation_fee_per_person: z.number().int().min(0).max(10_000_000).optional(),
...
const toCreateCommand = (input: z.infer<typeof createRoomSchema>): CreateRoomInput => ({
  ...
  participationFeePerPerson: input.participation_fee_per_person,
...
const roomDto = (result: Awaited<ReturnType<RoomService['getRoom']>>) => ({
  ...
  participation_fee_per_person: result.room.participationFeePerPerson,
    ...
    try {
      ...
      response.json({
        data: await Promise.all(result.data.map(async (card) => ({
          ...
          participation_fee_per_person: card.participationFeePerPerson,
          preferred_skill: { min_score: card.preferredSkillMin, max_score: card.preferredSkillMax },
